...
const PERIPHERAL_POSITIONS_JSON: JsonPositions = JSON.parse(
    readFileSync("public/positions.json", { encoding: "utf8" })
);
const PERIPHERAL_POSITIONS: ParsedPositions = Object.fromEntries(
    Object.entries(PERIPHERAL_POSITIONS_JSON).map((ent) => {
...
export class NetworkTarget {
    ...
    triangulate(overridePositions?: ParsedPositions) {
        ...
        const eq1_answer =
            d2 ** 2 - d1 ** 2 + (p2.x ** 2 - p1.x ** 2 + p2.y ** 2 - p1.y ** 2);

        const eq2_answer =
            d3 ** 2 - d1 ** 2 + (p3.x ** 2 - p1.x ** 2 + p3.y ** 2 - p1.y ** 2);
        ...
        const x = (C_2 * B_1 - B_2 * C_1) / (A_2 * B_1 - B_2 * A_1);
